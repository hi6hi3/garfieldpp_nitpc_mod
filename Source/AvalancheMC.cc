...
void AvalancheMC::GetNegativeIonEndpoint(const unsigned int i, double& x0, double& y0,
                                 double& z0, double& t0, double& x1, double& y1,
                                 //double& z1, double& t1, int& status){
                                 double& z1, double& t1, int& status) const {
  if (i >= m_endpointsNegativeIons.size()) {
    std::cerr << m_className << "::GetNegativeIonEndpoint: Index out of range.\n";
    return;
  }

  x0 = m_endpointsNegativeIons[i].x0[0];
  y0 = m_endpointsNegativeIons[i].x0[1];
  z0 = m_endpointsNegativeIons[i].x0[2];
  t0 = m_endpointsNegativeIons[i].t0;
  x1 = m_endpointsNegativeIons[i].x1[0];
  y1 = m_endpointsNegativeIons[i].x1[1];
  z1 = m_endpointsNegativeIons[i].x1[2];
  t1 = m_endpointsNegativeIons[i].t1;
  status = m_endpointsNegativeIons[i].status;
}
...
//---------------------------------------------------------
// T.Shimada Develop
bool AvalancheMC::DriftNegativeIon(const double x0, const double y0, const double z0,
                                   const double t0) {
  if (!m_sensor) {
    std::cerr << m_className << "::DriftIon: Sensor is not defined.\n";
    return false;
  ...
  return DriftLine(x0, y0, z0, t0, 3);
}
//---------------------------------------------------------
...
// T.Shimada Develop
    ...
    // SF6 Negative Ion Detachment
    if (m_isIonDetachment && type == 3) {
      int flag_detach = DetachmentNegativeIon(m_dMc,medium,e0,v0,146,status);
      if(flag_detach!=-1)break;
    }
...
//-------------------------------------------------------------------
  ...
  unsigned int nNegativeIonsOld = m_nNegativeIons;
  ...
  if (type == -1) {
    ...
    m_endpointsNegativeIons.push_back(std::move(endPoint));
  }
  ...
  if (m_debug) {
    ...
    const int nNewNegativeIons = m_nNegativeIons - nNegativeIonsOld;
    std::cout << m_className << "::DriftLine: Produced\n"
              << "      " << nNewElectrons << " electrons,\n"
              << "      " << nNewHoles << " holes, and\n"
              << "      " << nNewIons << " ions.\n"
              << "      " << nNewNegativeIons << " negative ions.\n";
  }
  ...
  if (m_viewer && !m_drift.empty()) {
    ...
    if (type < 0) {
      ...
      m_viewer->NewNegativeIonDriftLine(nPoints, id, xi, yi, zi);
    } else {
...
//---------------------------------------------------------
// T.Shimada Develop
void AvalancheMC::InputDetachCrossSectionData(std::string filename)
{
...
double AvalancheMC::GetDetachCrossSection(double energy)
{
...
double AvalancheMC::ComputeLambda(double pressure, double x_sec)
{ 
  //pressure [Torr]
  double Temp = 300.; // K
...
void AvalancheMC::SetDetachModel(int model_num)
{
  DetachmentModelNo = model_num;
}

int AvalancheMC::DetachmentNegativeIon(const double step, Medium* medium,
                                       const std::array<double, 3>& e,
                                       const std::array<double, 3>& v,

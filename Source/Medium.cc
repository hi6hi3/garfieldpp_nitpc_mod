...
//--------------------
// T.Shimada Develop
void Medium::GetNegativeIonMomentum(const double e, const double niMass, double& px, double& py,
                                 double& pz, int& band) {
  const double p = sqrt(2. * niMass * e) / SpeedOfLight;
  RndmDirection(px, py, pz, p);
  band = -1;
}
//--------------------
...
/// T.Shimada Develop
///----------------------------------------------------------------------------
bool Medium::NegativeIonVelocity(const double ex, const double ey, const double ez,
                         const double bx, const double by, const double bz,
                         double& vx, double& vy, double& vz) {
  vx = vy = vz = 0.;
  if (m_iMob.empty()) return false;
  ...
  constexpr double q = -1.; // Negative Charge
  mu *= q;
  if (b < Small) {
    vx = mu * ex;
    vy = mu * ey;
...
bool Medium::NegativeIonDiffusion(const double ex, const double ey, const double ez,
                          const double bx, const double by, const double bz,
                          double& dl, double& dt) {
...
bool Medium::NegativeIonDissociation(const double ex, const double ey, const double ez,
                             const double bx, const double by, const double bz,
                             double& diss) {
